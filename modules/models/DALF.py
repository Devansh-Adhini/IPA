...
class DEAL(nn.Module):
  ...
  def __init__(self, enc_channels = [1, 32, 64, 128], fixed_tps = False, mode = None): 
    ...
    if self.mode == 'ts-fl':
      ...
      self.fusion_layer = nn.Sequential(nn.Linear(128, 256), nn.ReLU(), nn.Dropout(0.1),
  ...
  def soft_NMS(self, x, threshold = 3., kernel_size = 3, sigma = 0.5):
    """Soft-NMS: attenuate overlapping keypoints instead of zeroing them."""
    pad = kernel_size // 2
    local_max = nn.MaxPool2d(kernel_size=kernel_size, stride=1, padding=pad)(x)
    suppression_weight = torch.exp(-((x - local_max)**2) / (2 * sigma**2))
    suppressed = x * suppression_weight
    pos = (suppressed > threshold)
    return pos.nonzero()[..., 1:].flip(-1)
  ...
  def forward(self, x, NMS = False, threshold = 3., return_tensors = False, top_k = None):
    ...
    if not NMS:
      ...
      # Using Hard-NMS for fair benchmarking (Soft-NMS clusters points and lowers coverage)
    ...
    if NMS:
      for b in range(B):
        ...
        if self.mode == 'end2end-full' or self.mode == 'ts2' or self.mode == 'ts-fl':
          ...
          fd = self.interpolator(out['feat'][b], kpts[b]['xy'], H, W)
        else:
  ...
  def forward(self, x, in_imgs, keypts, Ho ,Wo):
    ...
    for b in range(B):
      if keypts[b]['xy'] is not None and len(keypts[b]['xy']) >= 16:
        N = len(keypts[b]['xy'])
        # Fully batch everything (Grid Gen + TPS + Sample) to avoid OOM
        chunk_size = 512
        curr_patches_list = []
        
        # Pre-calculate theta for all points in this image to avoid redundant fcn/interpolator calls
        all_theta = self.interpolator(Theta[b], keypts[b]['xy'], Ho, Wo) 
        all_theta = self.attn(all_theta)
        all_theta = all_theta.view(-1, self.nparam, 2)
        
        for i in range(0, N, chunk_size):
            chunk_xy = keypts[b]['xy'][i : i+chunk_size]
            chunk_theta = all_theta[i : i+chunk_size]
            ni = len(chunk_xy)
            
            # 1. Generate Polar Grid for this chunk ONLY
            polargrid_i = self.get_polar_grid(chunk_xy, Ho, Wo)
            Ni, Hi, Wi, _ = polargrid_i.shape

            # 2. Setup for TPS
            kfactor = 0.3
            offset = (1.0 - kfactor)/2.
            vmin = polargrid_i.view(Ni,-1,2).min(1)[0].unsqueeze(1).unsqueeze(1)
            vmax = polargrid_i.view(Ni,-1,2).max(1)[0].unsqueeze(1).unsqueeze(1)
            ptp = vmax - vmin
            polargrid_i = (polargrid_i - vmin) / ptp
            polargrid_i = polargrid_i * kfactor + offset

            grid_img_i = polargrid_i.permute(0,3,1,2) 
            ctrl_i = F.interpolate(grid_img_i, self.ctrlpts).permute(0,2,3,1).view(Ni,-1,2)

            I_polargrid_i = chunk_theta.new(Ni, Hi, Wi, 3) 
            I_polargrid_i[..., 0] = 1.0
            I_polargrid_i[..., 1:] = polargrid_i

            # 3. Solve TPS for this chunk
            ...
            # 4. Final Sampling for this chunk
            tps_warper_i = (tps_warper_i - offset) / kfactor        
            tps_warper_i = tps_warper_i * ptp + vmin

            p_chunk = F.grid_sample(in_imgs[b].expand(Ni,-1,-1,-1),
...
class HardNet(nn.Module):
  def __init__(self, nchannels=3, out_ch = 128):
    ...
    # SE channel attention after conv body
    self.se = SEBlock(64, reduction=4)

    self.pool_and_head = nn.Sequential(
  ...
  def forward(self, x):
    if x is not None:
      x = self.conv_body(x)
      x = self.se(x)
      x = self.pool_and_head(x).squeeze(-1).squeeze(-1)
      #x = F.normalize(x)
    return x
...
class SEBlock(nn.Module):
  ...
  def __init__(self, ch, reduction=4):
      ...
      self.fc = nn.Sequential(nn.Linear(ch, ch // reduction, bias=False),
                              nn.ReLU(),
...
class UNet(nn.Module):
    ...
    def __init__(self, enc_channels = [1, 32, 64, 128]):
      ...
      # SE attention on feature head for adaptive channel re-weighting
      self.se = SEBlock(enc_channels[-1])
    ...
    def forward(self, x):
      ...
      #feat = feats[-1]
      feat = self.se(self.features(feats[-1]))
